...
//angle the commanded angle for hold
...
void turnrobot(int angle, int direction, int speed)
{
	...
	while(abs(SensorValue[gyro] - desiredangle) > 5 )
	{
...
void moverobot(int xx1,int xx2,int yy1, int duration, int automode, int gospeed)
{
	int direction;
	int turndir;
	int gyroholdpos = SensorValue[gyro];
	int delta = 10;
	if (automode==0)
	{
	...
	if (automode==1)
	{
		if(yy1!=0)//code for movement forward or reverse.  +yy1=forward, -yy1=reverse
		{
			...
			/////////////////////////////////////////////////////////////////////////////////////
			// this while loop moves the base forward or reverse until the desired distance - yy1 - is achieved
			...
			
			{
				...
				//This next test will keep robot straight if moving back or forward.
				if (abs(SensorValue[gyro]) >= abs(gyroholdpos)+10)
				{
					//This tests for angle between desired drive direction and corrects for drift using the gyro turn robot function
				turndir = (gyroholdpos-SensorValue[gyro] < 0) ? 0 : 1;
					turnrobot(abs(gyroholdpos-SensorValue[gyro]), turndir, 120);
				}
		...
		if(xx1!=0)//code for movement left or right.  +xx1=right, -xx1=left
		{
			...
			while (( abs(nMotorEncoder[backLeft]) < (abs(xx1)-15))&&(abs(nMotorEncoder[frontLeft]) < (abs(xx1)-15)))
				//||((abs(nMotorEncoder[backLeft]) > abs(xx1+200)) && abs(nMotorEncoder[frontLeft]) > abs(xx1+200)))
			{
				...
				//if (SensorValue[gyro]!= gyroholdpos)
				//{
				//	//if this , then do this or this
				//  turndir = (gyroholdpos-SensorValue[gyro] < 0) ? 0 : 1;
				//	turnrobot(gyroholdpos/10, turndir, 120);
				//}
