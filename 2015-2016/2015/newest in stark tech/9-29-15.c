...
#pragma config(Motor,  port7,           RightShootT,   tmotorVex393_MC29, openLoop, driveRight, encoderPort, I2C_2)
...
void pre_auton()
{
	...
	SmartMotorSetFreeRpm(LeftShootT, 130);
	SmartMotorRun();
	//clear gyro
	SensorType[gyro] = 0;
	wait1Msec(50);
...
task usercontrol()
{
	...
	int X1 = 0, Y1 = 0, X2 = 0,Y2 = 0, sSpeed = 80, iSpeed = highspd;
	int threshold = 20;
	while (true)
	{
	...
	Y2 = (abs(vexRT[Ch3Xmtr2]) > threshold+10) ? (vexRT[Ch3Xmtr2]*2/3) : (0);
		...
		//shoot(highspd, 1, 100);
		smartIntake(iSpeed, sSpeed, 72, smOF);
		if (vexRT[Btn8D] == 1)
		{
			if(smOF == 0)
			{
