...
task lcd()
{
	...
	while(1==1)
	{
		...
		{
			speedR += mult;// right side more than 20 below the min RPM, then increase by 1
		}
		else if(SmartMotorGetSpeed(RightShootT) > minRPM + range)
		{
			speedR -= mult;// right side more than 20 above the max RPM, then decrease the right side by 1
		}
		//left side
		if(SmartMotorGetSpeed(LeftShootT) < minRPM - range)
		{
			speedL += mult;// left side more than 20 above the min RPM, then increase the right side by 1
		}
		else if(SmartMotorGetSpeed(LeftShootT) > minRPM + range)
		{
			speedL -= mult;// right side more than 20 above the max RPM, then decrease the right side by 1
		}
		if(speedL < 0)
		{
			speedL = 0;
