...
int max;
int min;
void shoot(int max2,int min2,int on_off,int  minRpm2)
{
	minRPMI = minRpm2;
	max = max2;
	min = min2;
...
void smartIntake(int iSpeed,int minRPM,int ShootOn_Off,int Smax,int Smin,)
{
	//		60				true				70
	shoot(Smax, Smin, ShootOn_Off, minRPM);
...
task lcd()
{
	...
	while(1==1)
	{
		if(SmartMotorGetSpeed(RightShootT) < minRPMI - 20)
		{
			speedR = speedR + 1;
		}
		else if(SmartMotorGetSpeed(RightShootT) > minRPMI + 20)
		{
			speedR = speedR - 1;
		}
		else if(SmartMotorGetSpeed(LeftShootT) < minRPMI - 20)
		{
			speedL = speedL + 1;
		}
		else if(SmartMotorGetSpeed(LeftShootT) > minRPMI + 20)
