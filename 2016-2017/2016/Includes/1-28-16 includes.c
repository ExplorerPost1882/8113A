...
void shoot(int speed,int on_off,int minRpm)
{
	...
	if(on_off == 0)
	{
		if(SmartMotorGetSpeed(RightShootT) > minRpm && SmartMotorGetSpeed(LeftShootT) > minRpm)//both are greater than rpm
		{
			if(SmartMotorGetSpeed(RightShootT) > SmartMotorGetSpeed(LeftShootT))//right > left, but > rmp
			{
				SetMotor(LeftShootT,speed);
				SetMotor(LeftShootB,speed);
				SetMotor(RightShootB,speed-15);
				SetMotor(RightShootT,speed-15);
			}
			else if(SmartMotorGetSpeed(RightShootT) < SmartMotorGetSpeed(LeftShootT))//left > right, but > rmp
			{
				SetMotor(LeftShootT,speed-15);
				SetMotor(LeftShootB,speed-15);
				SetMotor(RightShootB,speed);
				SetMotor(RightShootT,speed);
			}
			else if(SmartMotorGetSpeed(RightShootT) == SmartMotorGetSpeed(LeftShootT))//right = left, but > rmp
			{
				SetMotor(LeftShootT,speed);
				SetMotor(LeftShootB,speed);
				SetMotor(RightShootB,speed);
		...
		if(SmartMotorGetSpeed(RightShootT) < minRpm || SmartMotorGetSpeed(LeftShootT) < minRpm)//both are less than rpm
		{
			if(SmartMotorGetSpeed(RightShootT) > SmartMotorGetSpeed(LeftShootT))//right > left, but < rpm
			{
				SetMotor(LeftShootT,speed+25);//+25 to get up to speed to other wheel and rpm
				SetMotor(LeftShootB,speed+25);
				SetMotor(RightShootB,speed+15);//+15 to get up to speed to rpm
				SetMotor(RightShootT,speed+15);
			}
			else if(SmartMotorGetSpeed(RightShootT) < SmartMotorGetSpeed(LeftShootT))//left > right, but < rpm
			{
				SetMotor(LeftShootT,speed+15);
				SetMotor(LeftShootB,speed+15);
			...
			else if(SmartMotorGetSpeed(RightShootT) == SmartMotorGetSpeed(LeftShootT))//right = left, but < rpm
			{
				SetMotor(LeftShootT,speed+15);
				SetMotor(LeftShootB,speed+15);
				SetMotor(RightShootB,speed+15);
...
void smartIntake(int iSpeed,int sSpeed,int minRPM,int ShootOn_Off)
{
	//		60				true				70
