...
void pincers(int pncTP, int speed)
{
	while ((abs(nMotorEncoder[pincer]) != (abs(pncTP)-40)) || (abs(nMotorEncoder[pincer]) != (abs(pncTP)+40)))
	{
		if((abs(nMotorEncoder[pincer])) < (abs(pncTP)-30))
		{
			motor[pincer] = -speed;
			wait1Msec(1);
		}
		else if((abs(nMotorEncoder[pincer])) > (abs(pncTP)+30))
		{
			motor[pincer] = speed;
			wait1Msec(1);
		}
