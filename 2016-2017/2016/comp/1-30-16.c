...
#include "1-30-16 includes.c"				 //main user code has all the voids...no need to modify.
//speed
...
task autonomous()
{
	...
	while(1)
	{
		...
		smartIntake(iSpeed, minRPM, 0, sMax , sMin);
	}
...
task usercontrol()
{
	...
	Y2 = (abs(vexRT[Ch3Xmtr2]) > threshold+10) ? (vexRT[Ch3Xmtr2]*2/3) : (0);
		...
		smartIntake(iSpeed, minRPM, vexRT(Btn6U), sMax , sMin);
		if(vexRT(Btn8D) == 1)
		{
			sMax = nexNetMax;
			sMin = nexNetMin;
