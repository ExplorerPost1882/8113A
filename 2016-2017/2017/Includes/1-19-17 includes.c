...
void pincers(int speed, int pncTP, int autoP)
{
	if(autoP == 0)
	{
		motor[pincerR] = speed;
	...
	if(autoP ==1)
	{
		while ((abs(nMotorEncoder[pincerR]) < (abs(pncTP)-30)) || (abs(nMotorEncoder[pincerR]) > (abs(pncTP)+30)))
		{
			if(abs(nMotorEncoder[pincerR]) < abs(pncTP))
			{
				motor[pincerR] = speed;
				motor[pincerL] = speed;
			}
			else if(abs(nMotorEncoder[pincerR]) > abs(pncTP))
			{
				motor[pincerR] = -speed;
				motor[pincerL] = -speed;
			}
